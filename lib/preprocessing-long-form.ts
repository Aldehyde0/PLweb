...
function make(input: Input): LongFormConcept {
  return {
    id: input.slug,
    slug: input.slug,
    title: input.title,
    ...
    corePrinciple: input.principle,
    formulas: input.formulas ?? [],
    variableDefinitions: input.variables ?? [],
    numericalExample: input.numericalExample,
    algorithmSteps: input.steps,
    codeExamples: input.code,
    codeExplanation: input.code.flatMap((item) => item.explanation),
...
export const preprocessingLongForms: LongFormConcept[] = [
  make({
    slug: 'preprocessing-fit-transform-boundary',
    title: '预处理的 fit / transform 边界',
    summary:
      '先划分数据，只在训练数据上学习预处理参数，再把同一规则应用到验证、测试和未来数据。',
    definition: [
      '预处理中的 fit 会从数据学习状态，例如填补中位数、标准化均值与标准差、One-Hot 类别集合或 PCA 方向；transform 只使用已学状态转换输入。',
      '数据划分不是预处理之后的步骤，而是所有会学习参数的变换之前的边界。',
    ],
    background: [
      '许多评价泄漏并不发生在分类器内部，而发生在模型之前。即使测试标签未被使用，只要测试特征参与计算均值、类别词表或选择阈值，评价就不再独立。',
    ],
    intuition: [
      '把测试集想成封存试卷。fit 相当于总结试卷规律，只有训练材料允许被总结；验证和测试只能按已有规则作答。',
    ],
    principle: [
      '正确顺序是 split → fit preprocessing on train → transform train/validation/test。',
      '交叉验证时，每一折都拥有自己的训练边界，因此预处理器必须放进 Pipeline，让每个训练折独立 fit。',
    ],
    formulas: [
      {
        label: '数据边界',
        expression: 'θprep = fit(Xtrain)，Xsplit′ = transform(Xsplit; θprep)',
        description: 'θprep 只能由训练数据产生，所有数据分块复用它。',
      },
    ],
    variables: [
      {
        symbol: 'θprep',
        meaning: '预处理器学到的中位数、均值、类别表、主成分等状态。',
      },
      { symbol: 'Xsplit', meaning: '训练、验证、测试或线上输入之一。' },
    ],
    steps: [
      '在查看任何整体统计量前确定划分策略。',
      '保留最终测试集。',
    ...
    code: [
      {
        title: 'Pipeline 保证折内拟合',
        language: 'scikit-learn',
        purpose: '让填补、标准化和分类器共享正确的数据边界。',
        source: `from sklearn.impute import SimpleImputer\nfrom sklearn.linear_model import LogisticRegression\nfrom sklearn.pipeline import make_pipeline\nfrom sklearn.preprocessing import StandardScaler\n\nmodel = make_pipeline(\n    SimpleImputer(strategy="median"),\n    StandardScaler(),\n    LogisticRegression(max_iter=2000),\n)\nmodel.fit(X_train, y_train)\ny_pred = model.predict(X_test)`,
        explanation: [
          'fit 时三步都只看到训练集。',
          'predict 对测试集依次调用 transform、transform、predict。',
        ],
        expectedOutput:
          '得到长度与 y_test 相同的预测数组；测试集不会改变任何预处理状态。',
      },
    ],
    applications: [
      '缺失值填补',
    ...
    pitfalls: [
      '完整数据 fit_transform 后再划分',
      '测试集重新 fit',
      '手工处理导致训练与预测规则不一致',
    ],
    prerequisites: ['训练/验证/测试职责', 'fit、transform、predict'],
    related: [
    ...
    code: [
      {
        title: 'Pandas 数据审计起点',
        language: 'Python',
        purpose: '生成结构、缺失、重复、描述统计和标签分布。',
        source: `print(df.shape)\nprint(df.dtypes)\nprint(df.isna().sum())\nprint("duplicates:", df.duplicated().sum())\nprint(df.describe(include="all"))\nprint(df["target"].value_counts(dropna=False))`,
        explanation: [
          'shape 和 dtype 先验证结构契约。',
          '缺失、重复和标签分布需要分别检查。',
          'describe 发现异常后仍需回到业务含义确认。',
        ],
        expectedOutput:
          '输出行列数、各列类型、缺失计数、重复行数量、描述统计和每类样本数。',
      },
    ],
    applications: ['建模前数据验收', '训练/线上数据契约', '错误样本追踪'],
    pitfalls: [
      '只看均值不看单位和来源',
      '把所有极端值直接删除',
      '忽略标签定义变化',
    ],
    prerequisites: ['Pandas 基础', '任务定义'],
    related: [
      'missing-value-imputation',
      'robust-scaling-and-outliers',
      'future-information-leakage',
    ],
    sections: ['§2 数据质量检查', '§8 异常值', '§14 预处理检查清单'],
  }),
  make({
    ...
    code: [
      {
        title: '数值与类别列分别填补',
        language: 'scikit-learn',
        purpose: '使用 ColumnTransformer 为不同类型列建立独立规则。',
        ...
        explanation: [
          'median 对极端值通常比 mean 稳健。',
          'add_indicator 保留哪些数值原本缺失。',
          '测试集只 transform。',
        ],
        expectedOutput:
    ...
    applications: ['表格分类与回归', '医疗和问卷数据', '传感器缺测'],
    pitfalls: [
      '用完整数据算填补值',
      '把未知类别和缺失混为一谈',
      '不分析高缺失比例特征的业务含义',
    ],
    prerequisites: ['训练/测试边界', '数值与类别数据'],
    related: [
      'data-quality-audit',
  ...
  make({
    ...
    code: [
      {
        title: 'One-Hot 编码未知类别',
        language: 'scikit-learn',
        purpose: '训练时建立类别空间，预测时安全处理新类别。',
        source: `from sklearn.preprocessing import OneHotEncoder\n\nencoder = OneHotEncoder(\n    handle_unknown="ignore",\n    sparse_output=False,\n)\ntrain = [["red"], ["green"], ["blue"]]\ntest = [["green"], ["yellow"]]\nencoder.fit(train)\nprint(encoder.get_feature_names_out())\nprint(encoder.transform(test))`,
        explanation: [
          'fit 只从训练集学习 red/green/blue。',
          'yellow 未见过，因此对应所有已知类别列为 0。',
        ],
        expectedOutput:
          '列名包含 blue、green、red；green 行有一个 1，yellow 行为全 0。',
      },
    ],
    applications: ['无序类别特征', '线性模型', '混合类型 Pipeline'],
    pitfalls: [
      '无序类别直接编码为 0/1/2',
      '训练和预测独立 get_dummies 导致列不一致',
      '忽略高基数问题',
    ],
    prerequisites: ['类别变量', '训练/测试边界'],
    related: [
      'missing-value-imputation',
      'sklearn-pipeline-column-transformer',
      'feature-construction',
    ],
    sections: ['§4 类别编码', '§13 同时处理数值和类别列'],
  }),
  make({
    slug: 'min-max-scaling',
    title: 'Min-Max 归一化',
    summary: '使用训练集最小值与最大值把特征线性映射到指定范围，典型为 [0,1]。',
    definition: [
    ...
    formulas: [
      {
        label: 'Min-Max 公式',
        expression: "x'ⱼ = (xⱼ - xⱼ,min) / (xⱼ,max - xⱼ,min)",
        description: '默认映射到 [0,1]；分母为 0 时是常量特征。',
      },
    ],
    variables: [
      { symbol: 'xⱼ,min', meaning: '第 j 列训练集最小值。' },
      { symbol: 'xⱼ,max', meaning: '第 j 列训练集最大值。' },
    ],
    numericalExample: {
      title: '把 [10,20,30] 映射到 [0,1]',
      given: ['训练最小值 10', '训练最大值 30'],
      steps: [
        '10 → (10-10)/(30-10)=0',
        '20 → 0.5',
        '30 → 1',
      ...
      result: '训练集变为 [0,0.5,1]，未来值可以超过目标范围。',
      comparison: [
    ...
    code: [
      {
        title: 'MinMaxScaler',
        language: 'scikit-learn',
        purpose: '安全保存训练区间并转换未来数据。',
        source: `from sklearn.preprocessing import MinMaxScaler\n\nscaler = MinMaxScaler()\ntrain = [[10.0], [20.0], [30.0]]\nfuture = [[40.0]]\nprint(scaler.fit_transform(train).ravel())\nprint(scaler.transform(future).ravel())`,
        explanation: [
          'fit_transform 只用于训练数据。',
          'future 超出训练最大值，因此得到 1.5。',
        ],
        expectedOutput: '[0. 0.5 1.]，随后 [1.5]。',
      },
    ],
    applications: ['有明确有限输入范围的模型', '像素或传感器特征缩放'],
    pitfalls: [
      '以为未来结果一定在 [0,1]',
      '存在异常值仍直接使用',
      '测试集重新拟合端点',
    ],
    prerequisites: ['最大值与最小值', '训练数据边界'],
    related: ['numerical-standardization', 'robust-scaling-and-outliers'],
    sections: ['§5 Min-Max 归一化'],
  }),
  make({
    slug: 'robust-scaling-and-outliers',
    title: 'Robust Scaling 与异常值处理',
    difficulty: '进阶',
    summary:
    ...
    formulas: [
      {
        label: '稳健缩放',
        expression: "x' = (x - mediantrain) / IQRtrain",
        description: 'IQR 是训练集第 75 百分位与第 25 百分位之差。',
      },
    ],
    variables: [
      { symbol: 'Q1', meaning: '训练集第 25 百分位。' },
      { symbol: 'Q3', meaning: '训练集第 75 百分位。' },
      { symbol: 'IQR', meaning: 'Q3-Q1，中间 50% 数据的跨度。' },
    ],
    numericalExample: {
      title: '极端值如何影响尺度',
      given: ['数据 [10,11,12,13,1000]'],
      steps: [
    ...
    code: [
      {
        title: 'RobustScaler 与 StandardScaler 对比',
        language: 'scikit-learn',
        purpose: '观察极端值对两种缩放器的影响。',
        source: `import numpy as np\nfrom sklearn.preprocessing import RobustScaler, StandardScaler\n\nX = np.array([[10.], [11.], [12.], [13.], [1000.]])\nprint(StandardScaler().fit_transform(X).ravel().round(3))\nprint(RobustScaler().fit_transform(X).ravel().round(3))`,
        explanation: [
          'StandardScaler 使用均值和标准差。',
          'RobustScaler 使用中位数和 IQR。',
        ],
        expectedOutput:
          '两组不同缩放值；RobustScaler 下普通样本间的差异不会被 1000 压缩得同样严重。',
      },
    ],
    applications: ['长尾金额', '含故障尖峰的传感器', '稳健线性/距离模型'],
    pitfalls: [
      '看到异常就删除',
      '把 RobustScaler 当异常检测器',
      '用全数据分位数设截断阈值',
    ],
    prerequisites: ['中位数与四分位数', '数据质量检查'],
    related: [
      'data-quality-audit',
      'numerical-standardization',
      'min-max-scaling',
    ...
    sections: ['§5 数值标准化与特征缩放', '§8 异常值'],
  }),
  make({
    slug: 'learning-rate-selection',
    title: '学习率选择',
    difficulty: '进阶',
    summary:
    ...
    formulas: [
      {
        label: '梯度更新',
        expression: 'θₜ₊₁ = θₜ - α∇J(θₜ)',
        description: 'α 控制一步移动距离。',
      },
    ],
    variables: [
      { symbol: 'α', meaning: '学习率。' },
      { symbol: '∇J', meaning: '当前参数处损失上升最快方向。' },
    ],
    steps: [
      '先缩放数值特征。',
      '选择对数间隔候选值。',
    ...
    code: [
      {
        title: '按对数尺度比较学习率',
        language: 'NumPy',
        purpose: '记录不同 α 的损失轨迹。',
        source: `candidate_rates = [0.0001, 0.001, 0.01, 0.1, 1.0]\nfor alpha in candidate_rates:\n    _, _, losses = gradient_descent(X_scaled, y_train, alpha)\n    print(alpha, losses[-1], np.isfinite(losses).all())`,
        explanation: [
          '使用相同的已标准化训练数据。',
          '有限性检查可发现 inf/nan 发散。',
        ],
    ...
    applications: ['线性回归梯度下降', '神经网络优化'],
    pitfalls: [
      '特征未缩放就比较学习率',
      '只跑固定轮数不看曲线',
      '用测试集挑 α',
    ],
    prerequisites: ['梯度下降', '数值标准化'],
    related: [
      'numerical-standardization',
      'gradient-descent-convergence',
      'gradient-descent',
    ],
    sections: ['§6 学习率的选择', '§7 判断梯度下降是否收敛'],
  }),
  make({
    ...
    formulas: [
      {
        label: '相对损失变化',
        expression: 'rₜ = |Jₜ-Jₜ₋₁| / max(|Jₜ₋₁|, ε)',
        description: '连续若干轮小于 tolerance 时可提前停止。',
      },
    ],
    variables: [
      { symbol: 'ε', meaning: '防止分母为零的小常数。' },
      { symbol: 'patience', meaning: '允许连续稳定或未改善的轮数。' },
    ],
    steps: [
      '每轮记录训练损失。',
      '检查 loss 和参数是否有限。',
    ...
    code: [
      {
        title: '耐心机制判断稳定',
        language: 'Python',
        purpose: '避免一次偶然的小变化导致过早停止。',
        source: `tolerance = 1e-8\npatience = 10\nstable_count = 0\n\nrelative_change = abs(old_loss - new_loss) / max(abs(old_loss), 1e-12)\nstable_count = stable_count + 1 if relative_change < tolerance else 0\n\nif stable_count >= patience:\n    print("梯度下降已经收敛")`,
        explanation: [
          '使用相对变化适配不同损失量级。',
          'stable_count 在变化重新变大时清零。',
        ],
        expectedOutput: '连续 10 轮相对变化小于 1e-8 后打印收敛提示。',
      },
    ],
    applications: ['手写梯度下降', '早停诊断', '优化器调试'],
    pitfalls: ['把训练收敛等同泛化成功', '不设最大轮数', '忽略 nan/inf'],
    prerequisites: ['损失函数', '学习率'],
    related: ['learning-rate-selection', 'overfitting-diagnosis'],
    sections: ['§7 判断梯度下降是否收敛'],
  }),
  make({
    ...
    formulas: [
      {
        label: 'Precision',
        expression: 'TP/(TP+FP)',
        description: '预测为正的样本中有多少是真的。',
      },
      {
        label: 'Recall',
        expression: 'TP/(TP+FN)',
        description: '真实正类中找到了多少。',
      },
      {
        label: 'F1',
        expression: '2PR/(P+R)',
        description: 'Precision 与 Recall 的调和平均。',
      },
    ],
    steps: [
      '统计每类数量。',
    ...
    code: [
      {
        title: '类别权重与分类报告',
        language: 'scikit-learn',
        purpose: '训练时提高少数类权重并查看逐类结果。',
        source: `from sklearn.linear_model import LogisticRegression\nfrom sklearn.metrics import classification_report\n\nmodel = LogisticRegression(class_weight="balanced", max_iter=2000)\nmodel.fit(X_train, y_train)\ny_pred = model.predict(X_test)\nprint(classification_report(y_test, y_pred))`,
        explanation: [
          'balanced 根据训练类别频率设置权重。',
          '报告 Precision、Recall、F1 和 support。',
        ],
        expectedOutput:
          '每个类别各自的 Precision、Recall、F1，以及 macro/weighted 平均。',
      },
    ],
    applications: ['欺诈检测', '疾病筛查', '故障告警'],
    pitfalls: ['只报告 Accuracy', '先对完整数据重采样', '用测试集选阈值'],
    prerequisites: ['混淆矩阵', '分类指标'],
    related: [
      'classification-metrics',
      'baseline-models',
      'cv-pipeline-leakage',
    ],
    sections: ['§9 类别不平衡', '§14 预处理检查清单'],
  }),
  make({
    slug: 'feature-construction',
    title: '领域、比率、多项式与交互特征',
    difficulty: '进阶',
    summary:
      '把原始字段组合为更贴近任务机制的特征，让简单模型也能表达有意义的非线性关系。',
    definition: ['特征构造是定义映射 φ(x)，把原始输入转为模型实际学习的表示。'],
    background: [
    ...
    formulas: [
      {
        label: '特征映射',
        expression: 'ŷ = wᵀφ(x)+b',
        description: '模型实际学习转换后的特征。',
      },
      {
        label: '二阶示例',
        expression: 'ŷ=w₀+w₁x₁+w₂x₂+w₃x₁x₂+w₄x₁²',
        description: '交互项表达共同作用，平方项表达曲率。',
      },
    ],
    steps: [
      '从任务机制提出候选特征。',
    ...
    code: [
      {
        title: '构造比率和二阶特征',
        language: 'Python',
        purpose: '展示领域比率与 PolynomialFeatures。',
        ...
        explanation: [
          '分母零的业务语义需要明确。',
          'PolynomialFeatures 在训练集 fit，测试集只 transform。',
        ],
        expectedOutput:
          '领域特征新增平均客单价；二阶展开产生原特征、平方项和两两交互项。',
      },
    ],
    applications: ['房价比率特征', '用户行为强度', '非线性回归'],
    pitfalls: [
      '构造预测时不存在的特征',
      '高阶展开导致维度爆炸',
      '相关性被误当因果',
    ],
    prerequisites: ['线性模型', 'Pipeline'],
    related: [
      'skewed-and-cyclical-features',
      'future-information-leakage',
      'feature-selection',
    ],
    sections: ['§10 特征工程与特征构造'],
  }),
  make({
    slug: 'skewed-and-cyclical-features',
    title: '偏态与周期特征变换',
    difficulty: '进阶',
    summary:
    ...
    formulas: [
      {
        label: '长尾变换',
        expression: "x'=log(1+x)",
        description: '非负数据中压缩大值。',
      },
      {
        label: '周期编码',
        expression: 'xsin=sin(2πx/T)，xcos=cos(2πx/T)',
        description: 'T 是周期长度。',
      },
    ],
    steps: [
    ...
    code: [
      {
        title: '金额与小时变换',
        language: 'NumPy',
        purpose: '实现文档中的 log1p 和 24 小时周期编码。',
    ...
    applications: ['金额与访问次数', '小时、星期与季节'],
    pitfalls: [
      '对含负数数据直接 log1p',
      '只用 sin 或 cos 一列',
      '没有明确真实周期',
    ],
    prerequisites: ['对数函数', '三角函数'],
    related: ['feature-construction', 'robust-scaling-and-outliers'],
    sections: ['§10 偏态特征变换', '§10 日期和周期特征'],
  }),
  make({
    slug: 'future-information-leakage',
    title: '未来信息泄漏',
    difficulty: '进阶',
    summary: '输入包含预测时尚未发生的信息，会制造无法上线复现的虚高离线效果。',
    definition: [
    ...
    intuition: ['这不是模型聪明，而是题目把未来答案印在了当前特征上。'],
    principle: [
    ...
    code: [
      {
        title: '按时间构造过去窗口',
        language: 'Python',
        purpose: '表达特征截止时间必须早于预测时点。',
        source: `cutoff = prediction_time\nhistory = events[events["event_time"] < cutoff]\nfeatures = history.groupby("user_id").agg(\n    past_30d_count=("event_id", "count"),\n)`,
        explanation: [
          '先按预测时点截断，再聚合。',
          '真实项目还需限定过去 30 天的开始时间。',
        ],
        expectedOutput: '每个用户只由预测时点之前的事件生成特征。',
      },
    ],
    applications: ['信用风险', '用户流失', '时间序列预测'],
    pitfalls: [
      '随机划分掩盖时间泄漏',
      '使用目标发生后的字段',
      '全历史聚合越过截止点',
    ],
    prerequisites: ['训练测试划分', '时间数据'],
    related: [
      'data-leakage',
      'time-series-cross-validation',
      'feature-construction',
    ],
    sections: ['§10 最危险的特征：未来信息', '§14 预处理检查清单'],
  }),
  make({
    ...
    code: [
      {
        title: 'Pipeline 内使用 L1 选择',
        language: 'scikit-learn',
        purpose: '避免先看完整数据再选择特征。',
        ...
        expectedOutput: '模型只把被训练折 L1 选择器保留的列传给最终分类器。',
      },
    ],
    applications: ['高维表格', '稀疏文本', '降低采集成本'],
    pitfalls: [
      '完整数据选择后再 CV',
      '把重要性解释为因果',
      '高度相关特征选择不稳定',
    ],
    prerequisites: ['交叉验证', '正则化'],
    related: [
      'l1-regularization',
      'feature-construction',
      'cv-pipeline-leakage',
    ],
    sections: ['§11 特征选择', '§12 使用 Pipeline 防止泄漏'],
  }),
  make({
    slug: 'sklearn-pipeline-column-transformer',
    title: 'Pipeline 与 ColumnTransformer',
    difficulty: '进阶',
    summary:
    ...
    code: [
      {
        title: '混合列完整 Pipeline',
        language: 'scikit-learn',
        purpose: '实现文档中的数值和类别双分支。',
    ...
    applications: ['混合类型表格', '交叉验证与调参', '模型保存交付'],
    pitfalls: [
      '手工对训练和测试分别 get_dummies',
      '忘记未知类别',
      '只保存分类器不保存预处理',
    ],
    prerequisites: ['缺失值', '类别编码', '标准化'],
    related: [
    ...
    sections: ['§12 使用 Pipeline 防止泄漏', '§13 同时处理数值和类别列'],
  }),
];
