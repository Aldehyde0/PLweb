    ...
    <main className="plan-page">
      <div className="plan-container">
        ...
        <header className="plan-page-header">
          <div>
            <p className="eyebrow">本地学习规划</p>
            <h1>学习计划</h1>
            <p>
              把现有知识目录组织成可调整的阶段和任务。计划数据只保存在当前浏览器。
            </p>
          </div>
          <Button size="lg" render={<Link href="/plans/new" />}>
            <Plus />
            创建学习计划
          </Button>
        </header>
        {!ready ? (
          ...
          <section className="plan-card-list" aria-label="已保存计划">
            {plans.map((plan) => {
              const method =
                PLAN_METHODS.find((item) => item.value === plan.method)
                  ?.label ?? plan.method;
              ...
              return (
                <article className="plan-card" key={plan.id}>
                  ...
                  <Progress
                    ...
                    aria-label={`${plan.title}总体进度 ${plan.completionRate}%`}
                  />
                  <div className="plan-card-meta">
                    <span>
                      <CalendarDays />
                      预计 {plan.estimatedCompletionDate}
                    </span>
                    <span>
                      <Clock3 />
                      每周 {plan.weeklyMinutes} 分钟
                    </span>
                    <span>
